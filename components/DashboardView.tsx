...
export const DashboardView = ({ evaluations, onBackToForm, onClearAll }: DashboardViewProps) => {
  ...
  const overallAverageScore = averageScores.reduce((acc, curr) => acc + curr.averageScore * curr.count, 0) / averageScores.reduce((acc, curr) => acc + curr.count, 0) || 0;
  const overallAverageGrade = filteredEvaluations.reduce((sum, ev) => sum + ev.finalGrade, 0) / filteredEvaluations.length || 0;
        ...
        <div>
            <p className="text-sm font-medium text-slate-600">Promedio de Puntaje</p>
             <p className="text-3xl font-bold text-purple-600">
                ...
                <span className="text-lg font-normal text-slate-500"> / {GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR.toFixed(1)}
            </span>
            </p>
        </div>
        <div>
            <p className="text-sm font-medium text-slate-600">Promedio de Nota Final</p>
            <p className="text-3xl font-bold text-green-600">{overallAverageGrade.toFixed(1)}</p>
        </div>
      </div>
                    ...
                    <tbody className="text-slate-700">
                        {[...filteredEvaluations].reverse().map(ev => (
                            <tr key={ev.id} className="hover:bg-slate-50">
                                ...
                                <td className={`px-6 py-4 whitespace-nowrap border-b border-slate-200 font-medium ${ev.finalGrade < GRADING_CONSTANTS.PASSING_GRADE ? 'text-red-600' : 'text-green-600'}`}>{ev.finalGrade.toFixed(1)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
        ...
        {averageScores.map(avg => (
          <div key={avg.indicatorId} className="p-4 border border-slate-200 rounded-lg bg-slate-50">
            <div className="flex justify-between items-center">
              ...
              <span className={`font-bold text-lg ${avg.averageScore >= (GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR * 0.75) ? 'text-green-600' : avg.averageScore >= (GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR * 0.5) ? 'text-yellow-600' : 'text-red-600'}`}>
                {avg.averageScore.toFixed(1)} / {GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR.toFixed(0)}
              </span>
            </div>
            <div className="w-full bg-slate-200 rounded-full h-2.5 mt-2">
              <div 
                className={`h-2.5 rounded-full ${avg.averageScore >= (GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR * 0.75) ? 'bg-green-500' : avg.averageScore >= (GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR * 0.5) ? 'bg-yellow-500' : 'bg-red-500'}`}
                style={{ width: `${(avg.averageScore / GRADING_CONSTANTS.MAX_POSSIBLE_SCORE_PER_INDICATOR) * 100}%` }}
              ></div>
            </div>
